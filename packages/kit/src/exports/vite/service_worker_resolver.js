...
export function create_service_worker_resolver(manifest_data) {
	...
	const route_data = routes.map((route, i) => {
		...
		const leaf = /** @type {import('types').PageNode} */ (route.leaf);
		const name = `worker_${i}`;
		imports.push(`import * as ${name} from ${s(`/${leaf.worker}`)};`);
		...
		return dedent`
			{
				...
				nodes: ${s([...page.layouts, page.leaf])},
				leaf: ${page.layouts.length},
				worker: ${name}
			}
		`;
	});
	...
	return dedent`
		...
		export async function resolve(event, options = {}) {
			...
			try {
				...
				const response = await resolve_worker_data(request, error);
				if (response) return response;
				throw error;
			}
		}

		async function resolve_worker_data(request, error) {
			...
			if (!invalidated[route_match.route.leaf]) return null;
			...
			const data = await route_match.route.worker.load({
				request,
				url: page_url,
				route: { id: route_match.route.id },
				params: route_match.params,
				network: get_network_state(error),
				invalidated,
				parent: async () => ({}),
				server: (options) => fetch_with_options(request, options)
			});

			const nodes = route_match.route.nodes.map((_, i) => {
				if (i !== route_match.route.leaf) return invalidated[i] ? null : { type: 'skip' };
				return {
					type: 'data',
					data,
					uses: {}
		...
		function render_data_response(nodes) {
			return '{"type":"data","nodes":[' + nodes.map((node) => {
				...
				return '{"type":"data","data":' + devalue.stringify(node.data) + ',"uses":{}}';
			}).join(',') + ']}\\n';
		}

		function parse_invalidated(value, length) {
			if (!value) return Array.from({ length }, () => true);
			return value.split('').map((char) => char === '1');
		}
